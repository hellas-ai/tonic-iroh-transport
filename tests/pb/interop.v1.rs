...
pub mod interop_service_client {
    #![allow(
        unused_variables,
        dead_code,
        missing_docs,
        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + std::marker::Send,
    {
        ...
        pub fn with_interceptor<F>(
            inner: T,
            interceptor: F,
        ) -> InteropServiceClient<InterceptedService<T, F>>
        where
...
pub mod interop_service_server {
    #![allow(
        unused_variables,
        dead_code,
        missing_docs,
    ...
    #[async_trait]
    pub trait InteropService: std::marker::Send + std::marker::Sync + 'static {
        async fn unary(
            &self,
    ...
    /// Minimal service exercising all four gRPC interaction modes.
    #[derive(Debug)]
    pub struct InteropServiceServer<T> {
        inner: Arc<T>,
    ...
    impl<T, B> tonic::codegen::Service<http::Request<B>> for InteropServiceServer<T>
    where
        T: InteropService,
                ...
                "/interop.v1.InteropService/ClientStream" => {
                    ...
                    for ClientStreamSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as InteropService>::client_stream(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
                ...
                "/interop.v1.InteropService/BidiStream" => {
                    ...
                    for BidiStreamSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as InteropService>::bidi_stream(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
    ...
    impl<T> Clone for InteropServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "interop.v1.InteropService";
    impl<T> tonic::server::NamedService for InteropServiceServer<T> {
